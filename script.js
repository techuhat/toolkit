...
function showUploadSuccessToast(fileCount, toolName) {
    if (fileCount === 1) {
        showToast(`✅ File uploaded successfully for ${toolName}!`, 'success', 3000);
    } else {
        showToast(`🎉 ${fileCount} files uploaded successfully for ${toolName}!`, 'success', 3000);
    }
}

function showProcessingStartToast(toolName) {
    const messages = {
...
function showErrorToast(toolName, errorMessage) {
    showToast(`❌ ${toolName} failed: ${errorMessage}`, 'error', 4000);
}
...
// Global Toast Manager - Module pattern for better organization
const ToastManager = {
    activeToasts: new Map(),
    maxToasts: 5,
    defaultDuration: 3000,
    
    // Initialize toast containers and styles
    init() {
        this.ensureToastContainer();
        this.setupGlobalStyles();
    },
    
    // Check if toast was already shown (for one-time messages)
    wasShown(messageId) {
        if (!messageId) return false;
        return localStorage.getItem(`toast-shown-${messageId}`) === 'true';
    },
    
    // Mark toast as shown
    markAsShown(messageId) {
        if (messageId) {
            localStorage.setItem(`toast-shown-${messageId}`, 'true');
        }
    },
    
    // Show toast with advanced options
    show(message, type = 'success', options = {}) {
        ...
        // Check if this is a one-time message that was already shown
        if (showOnce && this.wasShown(messageId)) {
            return null;
        }
        ...
        const toast = this.createToast(message, type, toastId, duration, persistent);
        ...
        this.showToast(toast, toastId, duration, persistent);
        
        // Mark as shown if it's a one-time message
        if (showOnce && messageId) {
    ...
    createToast(message, type, toastId, duration, persistent) {
        // Use the main showToast function for consistency
        return showToast(message, type, duration, { persistent, messageId: null, showOnce: false });
    },
    
    ensureToastContainer() {
        // Create toast style element if it doesn't exist
        if (!document.getElementById('toast-styles')) {
            ...
            style.textContent = `
                ...
                /* Force visibility for all toast elements */
                .toast {
...
// Ensure toast container styles are properly set
function ensureToastContainer() {
    // Simple container check - no complex logic needed
}

// Toast function removed - using only index.html implementation

// Simple reference to index.html showToast function
function showToast(message, type = 'success', duration = 4000) {
    // Use the toast function from index.html
    if (window.originalShowToast) {
        return window.originalShowToast(message, type, duration);
    }
    
    // Fallback if index.html function not available
    console.log(`Toast: ${message} (${type})`);
}

// Mobile touch support for swipe-to-dismiss
function addTouchSupport(toast, toastId) {
    let startX = 0;
    let currentX = 0;
    ...
    toast.addEventListener('touchmove', (e) => {
        ...
        if (diffY < 30 && diffX > 0) {
            ...
            toast.style.transform = `translateX(${diffX}px) translateY(-5px)`;
    ...
    toast.addEventListener('touchend', (e) => {
        ...
        // Dismiss if swiped far enough horizontally
...
// Enhanced dismiss function with better exit animation
// Enhanced dismiss function with better exit animation
function dismissToast(toastId) {
    ...
    if (toast) {
        ...
        // Enhanced exit animation to top-right
        toast.style.transition = 'all 0.3s ease-out';
        toast.style.transform = 'translateX(100%) translateY(-20px)';
        toast.style.opacity = '0';
        toast.style.scale = '0.95';
...
// All toast functions removed - using only index.html implementation
...
function downloadAllFiles(files, baseName = 'processed') {
    if (files.length === 1) {
        downloadFile(files[0].blob, files[0].name);
    } else if (files.length <= 5) {
        // Show multiple download notification
        showToast(`📁 Downloading ${files.length} files...`, 'success', 2000);
        ...
        showToast(`📁 Downloading ${files.length} files in sequence...`, 'success', 3000);
    ...
    processBtn.onclick = async function() {
        ...
        try {
            ...
            for (let i = 0; i < imageFiles.length; i++) {
                ...
                pdf.addImage(imageDataUrl, imageFormat, x, y, finalWidth, finalHeight);
            }
            ...
            downloadBtn.onclick = function() {
                downloadAllFiles(processedFiles);
                showToast('✅ PDF download started!', 'success', 2000);
            };
...
function setupImageResizer() {
    ...
    if (widthInput && heightInput && maintainAspectCheckbox) {
        ...
        widthInput.addEventListener('input', function() {
            ...
            // Always auto-calculate height when width changes (even without checkbox)
            if (!isUpdating && originalRatio && this.value && this.value > 0) {
                isUpdating = true;
